...
// Default fallback quests if quests.json doesn't exist
const DEFAULT_QUESTS = [
  {
    code: "QUEST_5S_INTRO",
    title: "Introduction to 5S",
    description: "Learn the fundamentals of 5S methodology",
    leanConcept: "5S",
    story: "A mysterious factory awaits your expertise. Start by understanding 5S basics.",
    objectives: ["Read 5S definition", "Identify muda", "Pass quiz"],
    difficulty: "easy",
    xpReward: 50,
    timeEstimate: 30,
    skillUnlock: null,
  ...
  },
  {
    ...
    description: "Master root cause analysis with Ishikawa diagram",
    leanConcept: "Problem Solving",
    story: "A quality issue plagues the assembly line. Can you find the root cause?",
    objectives: ["Create fishbone", "Identify 3 root causes", "Propose solution"],
    difficulty: "medium",
    xpReward: 100,
    timeEstimate: 45,
    skillUnlock: null,
  ...
  },
  {
    ...
    title: "Kaizen Event",
    description: "Plan and execute a continuous improvement event",
    leanConcept: "Kaizen",
    story: "Lead your team through a rapid improvement cycle",
    objectives: ["Gather team", "Identify improvements", "Measure impact"],
    difficulty: "hard",
    xpReward: 150,
    timeEstimate: 60,
    skillUnlock: null,
...
async function seedQuests() {
  ...
  let questsData = DEFAULT_QUESTS;

  // Try to load from file, fallback to defaults if not found
  if (fs.existsSync(questsPath)) {
    try {
      const fileContent = fs.readFileSync(questsPath, "utf-8");
      questsData = JSON.parse(fileContent);
      console.log(`📚 Loaded quests from quests.json`);
    } catch (error) {
      console.warn(`⚠️  Could not parse quests.json, using defaults:`, error);
    }
  } else {
    console.warn(
      `⚠️  quests.json not found, using ${DEFAULT_QUESTS.length} default quests`
    );
  }
